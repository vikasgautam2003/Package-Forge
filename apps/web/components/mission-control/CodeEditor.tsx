...
export function CodeEditor({
  ...
  return (
    <div className="flex-1 border-r border-[#3b82f6]/30 bg-[#0A0D14] flex flex-col shadow-[0_0_25px_rgba(59,130,246,0.05)] overflow-hidden">
      {/* Tabs */}
      <div className="flex bg-[#04060A] border-b border-[#3b82f6]/30 overflow-x-auto no-scrollbar">
        {files.map((file) => (
          <div
            key={file.name}
            onClick={() => onSelectFile(file.name)}
            className={`px-4 py-2 text-xs cursor-pointer border-r border-[#3b82f6]/20 transition-all duration-150 whitespace-nowrap
              ${
                selectedFile === file.name
                  ? "bg-[#0A0D14] text-[#60a5fa] border-t-2 border-t-[#60a5fa] shadow-[0_0_10px_rgba(96,165,250,0.35)]"
                  : "text-gray-500 hover:bg-[#0c1320] hover:text-[#93c5fd] hover:shadow-[0_0_8px_rgba(147,197,253,0.15)]"
